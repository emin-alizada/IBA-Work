    ...
    $('.menu > li').hover(e => {
        ...
        e => {
            ...
            if($li.find('.nav-sub').length) {
                ...
                $('#js-overlay-body-animated').stop().fadeOut(duration);
                $navSubMenu.stop().fadeOut(duration);
                $navSubMenu.stop().fadeOut(duration, () => { isSubmenuOpen = false; });
            }
        });
};
...
const calculateCredit = (val,type) => {
    ...
    var warranty = $('#js-selectric-credit-0').val();
    var rate = $("#"+warranty).val();

    if(type == 'month'){
        term = val;
        princ = $('#js-range-slider-0').val();
    }else if(type == 'amount'){
        princ = val;
        term = $('#js-range-slider-01').val();
    }
